...
import { FaSearch, FaShoppingCart, FaUser, FaBars, FaTimes, FaUserShield } from "react-icons/fa";
...
const Navbar = () => {
  ...
  const handleAdminLogout = () => {
    const confirmLogout = window.confirm("Are you sure you want to logout from admin panel?");
          ...
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex items-center space-x-4 lg:space-x-6">
              ...
              <Link to="/cart" className="relative hover:text-[#3B2F2F] transition">
                ...
                {cartItemCount > 0 && (
                  <span className="absolute -top-2 -right-2 text-xs w-5 h-5 flex items-center justify-center bg-red-600 rounded-full text-white font-bold animate-pulse">
                    {cartItemCount}
                  </span>
                )}
              </Link>
              ...
              {isAdmin && (
                <Link
                  to="/admin/dashboard"
                  className="flex items-center gap-2 bg-[#3B2F2F] text-white px-3 py-1.5 rounded-lg text-sm font-semibold hover:bg-[#2A2323] transition"
                >
              ...
              {user || isAdmin ? (
                <div className="relative group">
                  <button className="flex items-center space-x-2 text-sm hover:text-[#3B2F2F] transition">
                    {isAdmin ? (
                      <FaUserShield className="text-lg text-[#3B2F2F]" />
                    ) : (
                  ...
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-lg shadow-lg opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 z-50">
                    {isAdmin ? (
                      <>
                        ...
                        >
                          <FaUserShield className="inline mr-2 text-[#3B2F2F]" />
                          Admin Dashboard
                        </Link>
                        <button
                ...
                <button
                  onClick={openModal}
                  className="text-sm hover:text-[#3B2F2F] transition whitespace-nowrap"
                >
                  Login | Register
                </button>
              )}
        ...
        >
          <div className="px-4 py-3 space-y-3 bg-gray-50 border-t">
            ...
            <Link
              to="/cart"
              onClick={closeMobileMenu}
              className="flex items-center gap-3 py-2 hover:text-[#3B2F2F] transition font-medium"
            >
            ...
            {isAdmin && (
              <Link
                to="/admin/dashboard"
                onClick={closeMobileMenu}
                className="flex items-center gap-2 py-2 hover:text-[#3B2F2F] transition font-medium text-[#3B2F2F]"
              >
                <FaUserShield />
                Admin Dashboard
              </Link>
            ...
            {user || isAdmin ? (
              <>
                ...
                {isAdmin && (
                  <div className="block py-2 text-[#3B2F2F] font-medium">
                    Logged in as Admin ({admin?.name})
                  </div>
                )}
                ...
                <button
                  onClick={() => {
                    closeMobileMenu();
                    isAdmin ? handleAdminLogout() : handleLogout();
                  }}
                  className="block w-full text-left py-2 hover:text-[#3B2F2F] transition font-medium text-red-600"
                >
